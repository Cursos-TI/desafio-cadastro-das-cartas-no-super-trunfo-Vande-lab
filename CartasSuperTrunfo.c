...
int main() {
  ...
  float dp =populacao/km;
  float pbc = (float)(pib/populacao);
  float dp1 =populacao1/km1;
  float pbc1 = (float)(pib1/populacao1);
  float SuperPoder =km+pib+pbc+dp;
  float SuperPoder1 =km1+pib1+pbc1+dp1;
...
printf("A carta 2 tem mais pontos turisticos\n");
  ...
  {
    printf("A densidsde populacional é igual nas 2 cartas\n");
  }else if (dp>dp1)
  {
   printf("A d.p da carta 1 e maior, porisso perde nesse quesito\n");
  }
  else{
    printf("A d.p da carta 2 é maior porisso perde nesse quesito\n");
  }
  SuperPoder>SuperPoder1? printf("A soma de td o superpoder maior e da carta 1\n"):printf("na soma total a carta 2 tem o superpoder maior");
